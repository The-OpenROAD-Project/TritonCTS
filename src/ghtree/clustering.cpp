...
clustering::clustering(const vector<pin*>& sinks, float xBranch, float yBranch) {
	for (unsigned i = 0; i < sinks.size(); ++i) {
		flops.push_back(new flop(sinks[i]->x, sinks[i]->y, sinks[i]));
...
//	cout << "Clustering sanity check\n"; 
//	std::cout << "# of sinks = " << flops.size() << "\n";
//	std::cout << "# of clusters = " << _means.size() << "\n";
//	std::cout << "Capacity per cluster" << CAP << "\n";
//	
...
//	cout << "Actual segment sizes after fix: ";
//	for (unsigned i = 0; i < _means.size() - 1; ++i) {
//		float currSegmentLength = std::abs(_means[i].first - _means[i + 1].first) +
//								  std::abs(_means[i].second - _means[i + 1].second);
//		cout << currSegmentLength << " "; 
...
//				float target_x = sum_x/clusters[i].size();
//				float target_y = sum_y/clusters[i].size();
//				float ratio = 1.0;
//				for (unsigned j = 0; j < 10; ++j) {
//					float dx = target_x - pre_x;
//					float dy = target_y - pre_y;
//					//cout << "Ratio = " << ratio << "\n";
//					pair<float, float> target = {pre_x + ratio*dx, pre_y + ratio*dy};
//					//cout << "-->" << target.first << " " << target.second <<
//					//	" vs " << sum_x/clusters[i].size() << " "  << sum_y/clusters[i].size() << "\n";
//					if (calcDist(target, branchPoint) > minDist && calcDist(target, branchPoint) < maxDist) {
//						means[i] = target;
//						delta += std::abs(ratio*dx) + std::abs(ratio*dy);
//						break;
//					} else {
//						ratio *= 0.1;
//					}
//				}
				...
				//cout << "- delta = " << delta << " " << means[i].first << " " << means[i].second << "\n"; 
            }
        }
...
void clustering::getClusters(vector<vector<pin*>>& newClusters) {
	newClusters.clear();
	newClusters.resize(this->clusters.size());
	for (unsigned i = 0; i < clusters.size(); ++i) {
		newClusters[i].resize(clusters[i].size());
		for(unsigned j = 0; j < clusters[i].size(); ++j) {
			newClusters[i][j] = clusters[i][j]->p;
		} 
	}
}
